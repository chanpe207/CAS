...
%             paper_h = trisurf(f,v(:,1)+ obj.paperPickupCoords(1) ,v(:,3)+obj.paperPickupCoords(2), v(:,2)+obj.paperPickupCoords(3), 'FaceVertexCData', vertexColours, 'EdgeColor', 'interp');
        ...
        function MovetoPaperAuboi3Sim(obj, auboi3Robot, paperPose, paperModel)
            ...
            for i = 1:steps
                ...
                drawnow()
                pause(0.2)
                auboCollisionCheck(auboi3Robot,[obj.paperPickupCoords(1),obj.paperPickupCoords(2),obj.paperPickupCoords(3)],0.2)
            end
...
%         function auboChecksCollision(getauboi3,qMatrix,f,v,fn,returnwhenfound)%previously known as IsCollision
%             %robot - given the robot name --> getauboi3
%             %qMatrix - trajectory of getauboi3
%             %[f,v,fn] - of getUR3 OR table --> HARDCODE - REPLACE WITH MADE
%             %UP VALUES (WHEN STARTING OFF or IF TIME RUNS OUT)
%             %
%             
%             %example
%             %the final position at which auboi3 places the paper down
%             ..."[]" ==> hardcoded face normal value
%             if nargin < 6
%             returnwhenfound = true;
%             end
%             result = false;
%             
%             for qIndex = 1:size(qMatrix,1)
%                 tr = GetLinkPoses(qMatrix(qIndex,:),getauboi3);
%                 
%                 for i = 1:size(tr,3)-1
%                     for faceIndex = 1:size(faces,1)
%                         vertOnPlane = vertex(faces(faceIndex,1)',:);
%                         [intersectP,check] = LinePlaneIntersection
%                         
%                         %for table gotta change the swipe horizontal plane
%                         %to vertical swipe xy to yz
